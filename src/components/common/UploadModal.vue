...
import { format } from "date-fns"; // 用于时间格式化
import {
  ElMessage,
  ElLoading,
  ElButton,
...
export default {
  ...
  setup() {
        ...
        mapInstance.on("click", async (e) => {
          ...
          try {
            ...
            try {
              ...
              // 如果逆地理编码失败，使用经纬度作为地址
              form.location.address = `经度: ${lng.toFixed(
                6
              )}, 纬度: ${lat.toFixed(6)}`;
    ...
    const submitForm = async () => {
      ...
      //打印检查
      console.log("尝试提交-作品标题", form.title);
      console.log("尝试提交-作品分类", form.category);
      console.log("尝试提交-作品描述", form.description);
      console.log("尝试提交-作品经度", form.location.lng);
      console.log("尝试提交-作品纬度", form.location.lat);
      console.log("尝试提交-作品位置描述", form.location.address);
      console.log("尝试提交-作品路径", form.files);
      console.log("尝试提交-作品提交时间", form.ctime);
      console.log("尝试提交-作品作者", form.username);
